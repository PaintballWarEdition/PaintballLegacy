...
public class CmdAdmin {
	...
	public boolean command(final CommandSender sender, String[] args) {
		...
		if (args[1].equalsIgnoreCase("reset")) {
			...
			if (args.length == 3) {
				...
				plugin.playerManager.lookupPlayerUUIDForName(playerName, new Callback<UUID>() {
					...
					protected void onComplete(UUID uuid) {
						...
						plugin.statsManager.sendStats(sender, uuid, playerName);
					}
				});
			...
			if (args.length >= 3) {
				final String playerName = args[2];
				final String stat = (args.length == 3) ? "" : (args.length == 4 ? args[3] : null);
				if (stat == null) {
					return false;
				}
				...
				plugin.playerManager.lookupPlayerUUIDForName(playerName, new Callback<UUID>() {
					...
					protected void onComplete(UUID uuid) {
						...
						if (uuid != null && plugin.playerManager.exists(uuid)) {
							...
							if (stat.isEmpty()) plugin.statsManager.sendRank(sender, uuid, playerName, PlayerStat.POINTS);
							else plugin.statsManager.sendRank(sender, uuid, playerName, stat);
						} else {
							Map<String, String> vars = new HashMap<String, String>();
							vars.put("player", playerName);
							sender.sendMessage(Translator.getString("PLAYER_NOT_FOUND", vars));

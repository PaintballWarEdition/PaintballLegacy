					...
					for (Entity e : near) {
						if (e.getType() == EntityType.PLAYER) {
							...
							if (match == m && match.enemys(player, p)) {
								p.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20 * Paintball.getInstance().concussionSlownessDuration, 3));
								p.addPotionEffect(new PotionEffect(PotionEffectType.NAUSEA, 20 * Paintball.getInstance().concussionConfusionDuration, 3));
								p.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 20 * Paintball.getInstance().concussionBlindnessDuration, 3));
							}

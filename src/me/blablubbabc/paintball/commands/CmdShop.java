...
public class CmdShop {
	...
	public boolean command(CommandSender sender, String[] args) {
		if(sender instanceof Player) {
			...
			if(!plugin.shop) {
				player.sendMessage(gray + "Shop is inactive right now. :( See you later, alligator!");
				return true;
			}
			if(args.length == 1) {
				//Goods-List
				player.sendMessage(darkgreen+"$$$$$"+yellow+""+ ChatColor.BOLD+" Paintball-Shop "+darkgreen+"$$$$$");
				player.sendMessage("");
				int i = 1;
				for(String s : goods) {
					String msg = gold+String.valueOf(i)+gray+" : "+transformGood(s);
					if(player.hasPermission("paintball.shop.not"+String.valueOf(i)) && !player.isOp() && !player.hasPermission("paintball.admin")) msg = msg.concat(" "+red+"X");
					player.sendMessage(msg);
					i++;
				}
				player.sendMessage("");
				player.sendMessage(gold+"Buy with /pb shop [id]");
				return true;
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			} else if(args.length == 2) {
				if(plugin.mm.getMatch(player) == null) {
					player.sendMessage(gray+"The paintball-shop is only available while playing.");
					return true;
				}
				if(isNumber(args[1]) != null && isNumber(args[1]) > 0 && isNumber(args[1]) <= goods.size()) {
					if(transformGood(goods.get(isNumber(args[1])-1)).equalsIgnoreCase("empty") || (player.hasPermission("paintball.shop.not"+String.valueOf(isNumber(args[1]))) && !player.isOp() && !player.hasPermission("paintball.admin")) ) {
						player.sendMessage(gray+"This good is not available.");
						return true;
					}
					...
					if(cash < price) {
						player.sendMessage(gray+"$ Not enough cash $ :(");
						return true;
					}
					if(player.getInventory().firstEmpty() == -1) {
						player.sendMessage(gray+"Your inventory is full!");
						return true;
					}
					//money
					...
					player.sendMessage(green+"You bought "+amount+" "+split[1]+" for "+price+" $");

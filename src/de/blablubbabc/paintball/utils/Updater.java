    ...
    private boolean versionCheck(String title) {
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(" v").length == 2) {
                ...
                if (this.hasTag(version) || this.hasTag(remoteVersion) || version.equalsIgnoreCase(remoteVersion)) {
                    // We already have the latest version, or this build is tagged for no-update, or the latest file is tagged for no-update
